...
// create array of current search results 
// need to update to come from search API call
const books = [
	// need to add format for images somehow
...
const bookElement = <div>
	{books.map (book => (
		<div key={book.title}, className='book-cover', style=width: 128, height: 193>{book.image}</div>,
		<div key={book.title}, className='book-title'>{book.title}</div>,
		<div key={book.title}, className='book-authors'>{book.authors}</div>
	))}
</div>

// Render each book element
ReactDOM.render(
	bookElement, 
